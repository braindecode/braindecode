...
class LocalSelfAttention(nn.Module):
    ...
    ----------
    channels : int
        Input and output channel dimension. Must be divisible by ``heads``.
    radius : int, default=50
        Local window radius around each time point that participates in the
        attention computation.
    heads : int, default=4
        Number of attention heads. ``channels`` must be divisible by this value.
    """
