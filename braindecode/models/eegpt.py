...
class EEGPT(EEGModuleMixin, nn.Module):
    ...
    def __init__(
            ...
            # EEGPT specific parameters
            checkpoint=None,
            channel_names: List[str]=None,
            patch_size=64, # should depending on dataset
            patch_stride = 32, # should depending on dataset
            embed_num=4,
            embed_dim=512,
            depth=8,
            num_heads=8,
        ...
        self.chans_id    = target_encoder.prepare_chan_ids(channel_names)

        # -- load checkpoint
        if checkpoint:
            pretrain_ckpt = torch.load(checkpoint, weights_only=False)
        ...
        self.flattened_encoder_output_dim = target_encoder.num_patches[1] * embed_num * embed_dim
        self.final_layer = nn.Linear(self.flattened_encoder_output_dim, n_outputs) if not return_encoder_output else None
        
    def forward(self, x):
        z = self.target_encoder(x, self.chans_id.to(x)) # B, N, embed_num, D
        if self.return_encoder_output:
            return z

        h = z.flatten(1)
        if self.flattened_encoder_output_dim != h.shape[1]:
            raise ValueError(f"Expected output dim {self.flattened_encoder_output_dim}, got {h.shape[1]}")
