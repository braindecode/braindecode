...
if TYPE_CHECKING:
    import numpydantic as numpydantic_module
    import pydantic as pydantic_module
else:
    numpydantic_module = cast(Any, None)
    pydantic_module = cast(Any, None)

pydantic = _soft_import("pydantic", "model configuration", strict=False)
numpydantic = _soft_import("numpydantic", "model configuration", strict=False)
HAS_CONFIG_DEPS = pydantic is not False and numpydantic is not False

if HAS_CONFIG_DEPS:
    pydantic_module = cast(Any, pydantic)
    numpydantic_module = cast(Any, numpydantic)

if not HAS_CONFIG_DEPS:
    __all__: list[str] = []

    def __getattr__(name: str):
        raise ImportError(
            "Model config helpers require the optional pydantic and numpydantic "
            "dependencies. Install with: pip install braindecode[pydantic]"
        ) from None
else:
    import functools
    import operator
    ...
    def _replace_type_hints(type_hint: Any) -> Any:
        ...
        if origin is type or origin is Callable or type_hint is Callable:
            return pydantic_module.ImportString
        if origin is None:
            return type_hint
    ...
    def make_model_config(
        ...
        class BaseBraindecodeModelConfig(pydantic_module.BaseModel):
            def create_instance(self) -> EEGModuleMixin:
                kwargs = self.model_dump(mode="python", exclude={"model_name_"})
                if (
                    kwargs.get("n_chans") is not None
                    and kwargs.get("chs_info") is not None
                ):
                    kwargs.pop("n_chans")
                if (
        ...
        def validate_signal_params(cls, data: Any):
            ...
            if "n_outputs" in required and n_outputs is None:
                raise ValueError(
                    "n_outputs is a required parameter but was not provided."
                )
            ...
            if "chs_info" in required and chs_info is None:
                raise ValueError(
                    "chs_info is a required parameter but was not provided."
                )
        ...
        model_config = pydantic_module.create_model(
            f"{name}Config",
            model_name_=(Literal[name], name),
            __config__=pydantic_module.ConfigDict(
    ...
    BraindecodeModelConfig = Annotated[  # type: ignore
        ...
        pydantic_module.Field(discriminator="model_name_"),
    ]
