...
def update_label_index_dict(train_dataset):
    ...
    subset_aug_labels = [train_dataset[indice][1]
...
def global_variables_initialization(train_dataset):
    """Compute every global variable (label_index_dict, em_decomposition_dict,
     etc...)
...
def augment_dataset(train_dataset, subpolicies_list):
    """Transform the raw dataset into an augmented dataset. The Transform class
     does most of the job, nevertheless some additional work has to be done to
     adapt subset

    Args:
        train_dataset (Union[TransformDataset, TransformConcatDataset, Subset]):
        subpolicies_list (List[Union(Transforms, Compose)]): List of
        transforms/subpolicies (= composition of transforms) that should be
        applied on the dataset.
    """
    if isinstance(train_dataset, Subset):
        train_dataset.dataset.update_augmentation_policy(subpolicies_list)
        temp = [list(range(i, i + len(subpolicies_list)))
