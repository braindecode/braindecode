...
default_signal_params = dict(
    ...
    n_outputs=2,
    # These parameters will be inferred by EEGModuleMixin:
...
models_test_cases = [
    ...
    ('SleepStagerBlanco2020', ['n_chans', 'n_outputs', 'n_times'],
    ...
    ('SleepStagerEldele2021', ['n_outputs', 'n_times', 'sfreq'],
     dict(sfreq=100, n_times=3000, chs_info=[dict(ch_name='C1}', kind='eeg')])),  # 1 channel
...
def test_completeness__models_test_cases():
    ...
    assert all_models == models_tested, f"Models missing from models_test_cases: {all_models - models_tested}"


@pytest.mark.parametrize("model_name, required_params, signal_params", models_test_cases)
def test_model_integration(model_name, required_params, signal_params):
    ...
    assert set(required_params) <= {'n_times', 'sfreq', 'chs_info', 'n_outputs',
                                    'input_window_seconds', 'n_chans'}
    ...
    time_kwargs = []
    time_kwargs.append(dict(n_times=sp['n_times'], sfreq=sp['sfreq'], input_window_seconds=None))
    time_kwargs.append(
        dict(n_times=None, sfreq=sp['sfreq'], input_window_seconds=sp['input_window_seconds']))
    time_kwargs.append(
        dict(n_times=sp['n_times'], sfreq=None, input_window_seconds=sp['input_window_seconds']))
    if 'n_times' not in required_params and 'sfreq' not in required_params:
        time_kwargs.append(
            dict(n_times=None, sfreq=None, input_window_seconds=sp['input_window_seconds']))
    if 'n_times' not in required_params and 'input_window_seconds' not in required_params:
        time_kwargs.append(dict(n_times=None, sfreq=sp['sfreq'], input_window_seconds=None))
    if 'sfreq' not in required_params and 'input_window_seconds' not in required_params:
        time_kwargs.append(dict(n_times=sp['n_times'], sfreq=None, input_window_seconds=None))
    if 'n_times' not in required_params and 'sfreq' not in required_params and 'input_window_seconds' not in required_params:
        time_kwargs.append(dict(n_times=None, sfreq=None, input_window_seconds=None))
    model_kwargs_list = [dict(**o, **c, **t) for o in output_kwargs for c in channel_kwargs for t in
                         time_kwargs]
