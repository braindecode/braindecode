...
First, we install everything necessary for development

### Creating your own Braindecode fork
...
### Cloning your own fork locally

You can either clone via ssh or https, go to your personal fork on github.com to get the URLs, see
https://docs.github.com/en/github/creating-cloning-and-archiving-repositories/cloning-a-repository-from-github/cloning-a-repository for general intro to cloning a repository from Github,
here it may work like one of those (change <yourusername> to your GitHub user name):
```
...
Now we set the git configuration to allow us to interact with the upstream repository at github.com/braindecode/braindecode
...
``` 
(change the url for https if you want)
...
From within your local clone, run
```
...
pytest test/
```
This may take quite some  time on the first run, as some datasets will be downloaded. Subsequent executions will be faster.
...
### Finding an open issue
First, you can look  at the open issue under https://github.com/braindecode/braindecode/issues and find one that you feel interested.
Todo: tags
Make sure to comment that you try to work on that issue so that other people know about it.

### Creating a local branch for your contribution
To ensure you are up to date with the upstream master, in your local clone,
run:
```
git checkout master
...
Pick a local branch-name and within your local clone, run:
```
...
`test/unit_tests`. The directories below that should mirror the directory structure in braindecode.
To add your test, either insert your test to an existing test file if the tested module already has one or create a new corresponding test file.
...
In case you need, you may add another file there. Theses tests may also run a bit longer.
Make sure your test passes by running:
`pytest test/<yourtestfilepath>`
Also before pushing, make sure to run all tests to ensure they also still pass:
`pytest test`

#### Write Documentation

##### Add to API Documentation
In case your code adds some public function/class that can directly called by the user, please add it to our documentation as follows.
...
Also ensure to add your name at the top of the code file you edited, for example like:
```
...
These files will be executed during building the documentation.
...
make html
```
and check the built docuentation under `docs/_build/html/index.html`.
...
Open your fork on github.com, there should be a button to make a pull request to the main braindecode repository from your new branch.
Click that button and describe a bit more what you have changed so others can review your changes.
Now you should see your pull request appearing under https://github.com/braindecode/braindecode/pulls and the continuous integration will try to run the tests and built the documentation.
You can also then later add the number of your pull request to `docs/whats_new.rst`.
...
If you worked for a longer time on your code changes and the Braindecode master branch has already changed a lot,
it may become necessary to rebase your code changes on the current upstream master.
This will the git history look as if you just started your changes from the current master state.
To do that, from your local clone, first commit all the changes inside your local branch (without pushing) and then run the following:
```
git checkout master
git pull upstream master
...
To push, you may need to add `--force` like:
```
